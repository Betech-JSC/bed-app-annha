<template>
  ...
  <div class="crm-project-details-container min-h-screen bg-gray-50/10 p-2 lg:p-6">
    ...
    <div class="lg:grid lg:grid-cols-4 lg:gap-6 items-start">
              ...
              <template v-else-if="column.key === 'approver'">
                <div v-if="record.management_approver || record.accountant_approver" class="text-[10px] space-y-0.5">
                  ...
                  <div v-if="record.accountant_approver" class="flex items-center gap-1">
                    <CheckOutlined class="text-blue-500 text-[9px]" /> 
      ...
      <!-- ============ SUBCONTRACTOR INFO MAIN TAB ============ -->
      <a-tab-pane key="subcontractor_info" v-if="isTabVisible('subcontractor_info')">
        <template #tab><a-tooltip title="Quản lý thông tin chung nhà thầu phụ" placement="bottom">Thông tin chung</a-tooltip></template>
        <div class="p-4">
          <div class="flex justify-end mb-3">
          ...
          <a-table :columns="subInfoCols" :data-source="project.subcontractors || []" :pagination="{ pageSize: 10 }" row-key="id" size="small" class="crm-table" :custom-row="(r) => ({ onClick: () => openSubDetail(r), style: 'cursor: pointer' })">
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'name'">
                <div class="flex items-center gap-2">
      ...
      <a-tab-pane key="subcontractors" v-if="isTabVisible('subcontractors')">
        ...
        <div class="p-4">
          ...
          <a-empty v-if="!allSubcontractorPayments?.length" description="Chưa có chi phí thanh toán nào" />
        </div>
      </a-tab-pane>
          ...
          <template v-if="acceptanceGroups.length">
            <div v-for="group in acceptanceGroups" :key="group.parentTaskId" class="mb-6">
              ...
              <div v-for="acceptance in group.items" :key="acceptance.id"
                class="mb-3 rounded-xl border border-gray-100 bg-white overflow-hidden shadow-sm hover:shadow-md transition-all cursor-pointer hover:border-blue-300 group/card"
                ...
                <div class="flex items-center justify-between p-3">
                  <div class="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="font-semibold text-gray-800 text-[15px] truncate group-hover/card:text-blue-600 transition-colors">{{ acceptance.name }}</div>
                      <div class="flex items-center gap-2 mt-0.5">
                        <span v-if="acceptance.task?.name" class="text-[11px] text-gray-400 truncate">{{ acceptance.task.name }}</span>
                        <div v-if="acceptance.supervisor_approved_by || acceptance.customer_approved_by" class="flex gap-2 ml-1 border-l border-gray-200 pl-2">
                          <span v-if="acceptance.supervisor_approved_by" class="text-[10px] bg-gray-50 px-1.5 py-0.5 rounded text-gray-500 font-medium">GS: {{ acceptance.supervisorApprover?.name }}</span>
                          <span v-if="acceptance.customer_approved_by" class="text-[10px] bg-gray-50 px-1.5 py-0.5 rounded text-gray-500 font-medium">KH: {{ acceptance.customerApprover?.name }}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div v-if="getOpenDefects(acceptance) > 0 || (acceptance.workflow_status === 'rejected' && acceptance.rejection_reason)" 
                     class="px-3 pb-3 pt-0 flex flex-col gap-1.5">
                  <div v-if="getOpenDefects(acceptance) > 0" class="px-3 py-2 bg-red-50/80 rounded-lg flex items-start gap-2 cursor-pointer hover:bg-red-50 transition text-xs" @click.stop="openAcceptDetailModal(acceptance); activeTab = 'defects'">
                    <span class="text-red-500 mt-0.5"><WarningOutlined /></span>
                    <div>
                      <div class="font-semibold text-red-700">Còn {{ getOpenDefects(acceptance) }} lỗi chưa xử lý</div>
                      <div class="text-[11px] text-red-500/80 mt-0.5">Cần khắc phục để tiếp tục nghiệm thu</div>
                    </div>
                  </div>
                  <div v-if="acceptance.workflow_status === 'rejected' && acceptance.rejection_reason" class="px-3 py-2 bg-amber-50/80 rounded-lg flex items-start gap-2 text-xs text-amber-700">
                    <span class="text-amber-500 mt-0.5"><ExclamationCircleOutlined /></span>
                    <div>
                      <span class="font-semibold block mb-0.5">Lý do từ chối:</span>
                      <span class="text-amber-700/90 leading-tight">{{ acceptance.rejection_reason }}</span>
                    </div>
                  </div>
                </div>
              </div>
...
1277: 
      ...
      <div class="p-4 bg-white rounded-2xl border border-gray-100 shadow-sm">
        ...
        <div v-if="subDetail.payments?.length" class="space-y-2">
          <div v-for="p in subDetail.payments" :key="p.id"
               class="bg-gray-50 rounded-xl p-3 border border-gray-100 hover:border-blue-200 transition-all cursor-pointer"
               @click="openSubPaymentDetail(p, subDetail)">
            <div class="flex items-start justify-between">
              <div class="flex-1 min-w-0">
    ...
    <div v-if="selectedPurchase" class="space-y-6 pb-24">
      ...
      <div class="fixed bottom-0 right-0 w-[560px] p-4 bg-white/80 backdrop-blur-md border-t border-gray-100 flex justify-between items-center z-20">
         ...
         <div class="flex gap-2">
           ...
           <a-button v-if="selectedPurchase.status === 'pending_accountant'" type="primary" @click="confirmPurchaseKT(selectedPurchase)"><CheckOutlined /> KT Xác nhận</a-button>
    ...
    <div v-if="selectedUsage" class="space-y-6 pb-24">
      ...
      <div class="fixed bottom-0 right-0 w-[560px] p-4 bg-white/80 backdrop-blur-md border-t border-gray-100 flex justify-between items-center z-20">
         ...
         <div class="flex gap-2">
           ...
           <a-button v-if="selectedUsage.status === 'pending_return'" type="primary" @click="confirmReturn(selectedUsage)"><CheckOutlined /> Xác nhận trả</a-button>
         </div>
      </div>
    </div>
  </a-drawer>
  ...
  <a-drawer v-model:open="showSubPayDrawer" title="Ghi nhận thanh toán NTP" :width="520" @close="showSubPayDrawer = false" destroy-on-close class="crm-drawer">
     <div v-if="subDetail" class="bg-white rounded-xl p-4 mb-4 border border-gray-100">
        ...
        <div v-if="subDetail.payments?.length" class="space-y-3">
          <div v-for="p in subDetail.payments" :key="p.id" class="flex items-center justify-between py-2 border-b border-gray-50 last:border-b-0 cursor-pointer hover:bg-gray-50 transition-colors px-2 rounded" @click="openSubPaymentDetail(p, subDetail)">
            <div>
  ...
  <!-- ================= SUB PAYMENT DETAIL DRAWER ================= -->
  <a-drawer v-model:open="showSubPaymentDetailDrawer" title="Chi tiết Phiếu thanh toán" :width="560" @close="subPaymentDetail = null" destroy-on-close class="crm-drawer">
    <div v-if="subPaymentDetail" class="px-2">
      <!-- Status Badge -->
      <div class="flex justify-between items-start mb-6">
        <div>
          <div class="text-xs text-gray-500 font-medium mb-1">Mã tham chiếu: {{ subPaymentDetail.reference_number || '—' }}</div>
          <div class="text-2xl font-bold text-gray-800">{{ subPaymentDetail.payment_stage || 'Thanh toán' }}</div>
        </div>
        <a-tag :color="subPayStatusColors[subPaymentDetail.status]" class="px-3 py-1 rounded-full text-sm font-semibold border-0">{{ subPayStatusLabels[subPaymentDetail.status] || subPaymentDetail.status }}</a-tag>
      </div>

      <!-- Amount Box -->
      <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-5 mb-6 flex items-center justify-between shadow-sm">
        <div>
          <div class="text-xs font-bold text-blue-500 uppercase tracking-wider mb-1">Số tiền thanh toán</div>
          <div class="text-3xl font-black text-blue-700">{{ fmt(subPaymentDetail.amount) }}</div>
        </div>
        <div class="w-12 h-12 bg-white rounded-xl shadow-sm flex items-center justify-center text-blue-500 text-xl">
          💰
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 mb-6">
        <div class="bg-gray-50 rounded-xl p-3">
          <div class="text-[10px] text-gray-400 uppercase font-bold mb-1">Nhà thầu phụ</div>
          <div class="text-sm font-semibold text-gray-800">{{ subPaymentDetail.subcontractor?.name || subDetail?.name || '—' }}</div>
        </div>
        <div class="bg-gray-50 rounded-xl p-3">
          <div class="text-[10px] text-gray-400 uppercase font-bold mb-1">Phương thức</div>
          <div class="text-sm font-semibold text-gray-800">{{ subPayMethodLabels[subPaymentDetail.payment_method] || subPaymentDetail.payment_method }}</div>
        </div>
        <div class="bg-gray-50 rounded-xl p-3">
          <div class="text-[10px] text-gray-400 uppercase font-bold mb-1">Ngày thanh toán</div>
          <div class="text-sm font-semibold text-gray-800">{{ subPaymentDetail.payment_date ? fmtDate(subPaymentDetail.payment_date) : '—' }}</div>
        </div>
        <div class="bg-gray-50 rounded-xl p-3">
          <div class="text-[10px] text-gray-400 uppercase font-bold mb-1">Ngày tạo</div>
          <div class="text-sm font-semibold text-gray-800">{{ fmtDate(subPaymentDetail.created_at) }}</div>
        </div>
      </div>

      <!-- Notes -->
      <div v-if="subPaymentDetail.description" class="mb-6">
        <div class="text-xs font-bold text-gray-500 uppercase mb-2">Ghi chú / Diễn giải</div>
        <div class="bg-yellow-50/50 border border-yellow-100 rounded-xl p-3 text-sm text-gray-700">
          {{ subPaymentDetail.description }}
        </div>
      </div>
      ...
      <div class="mt-6 border-t border-gray-100 pt-6">
        <div class="text-xs font-bold text-gray-500 uppercase mb-4">Luồng phê duyệt</div>
        <a-steps direction="vertical" size="small" :current="getSubPayStepCurrent(subPaymentDetail.status)">
          <a-step title="Khởi tạo">
            <template #description>
              <div class="text-xs mt-1">
                <span v-if="subPaymentDetail.creator">Tạo bởi: <b>{{ subPaymentDetail.creator.name }}</b></span>
                <span v-else>Đã tạo phiếu</span>
              </div>
            </template>
          </a-step>
          <a-step title="Chờ BĐH duyệt" :status="subPaymentDetail.status === 'rejected' ? 'error' : (['pending_accountant_confirmation', 'paid'].includes(subPaymentDetail.status) ? 'finish' : (subPaymentDetail.status === 'pending_management_approval' ? 'process' : 'wait'))">
            <template #description>
               <div class="text-xs mt-1" v-if="subPaymentDetail.approver">
                  Duyệt bởi: <b>{{ subPaymentDetail.approver.name }}</b>
               </div>
               <div class="text-xs mt-1 text-red-500" v-if="subPaymentDetail.status === 'rejected' && subPaymentDetail.rejector">
                  Từ chối bởi: <b>{{ subPaymentDetail.rejector.name }}</b>
               </div>
            </template>
          </a-step>
          <a-step title="KT Xác nhận đã chi" :status="subPaymentDetail.status === 'paid' ? 'finish' : (subPaymentDetail.status === 'pending_accountant_confirmation' ? 'process' : 'wait')">
            <template #description>
               <div class="text-xs mt-1" v-if="subPaymentDetail.payer">
                  Chi bởi: <b>{{ subPaymentDetail.payer.name }}</b>
               </div>
            </template>
          </a-step>
        </a-steps>
    ...
    <!-- Action Footer -->
    <template #footer v-if="subPaymentDetail">
       <div class="flex justify-between items-center w-full">
          <!-- Left side actions (Revert, Delete) -->
          <div class="flex gap-2">
  ...
  <a-drawer v-model:open="showAcceptDetailDrawer" :width="560" :closable="false" destroy-on-close class="crm-drawer">
    <template #title>
      <div class="flex items-center justify-between w-full">
        <div class="flex items-center gap-2">
          ...
          <div>
            <div class="text-[10px] text-gray-400 uppercase font-bold leading-none mb-1">Chi tiết nghiệm thu</div>
            <div class="text-sm font-bold text-gray-800 leading-none truncate w-[280px]">{{ acceptDetailStage?.name }}</div>
          </div>
        </div>
        <div class="flex gap-1">
    ...
    <template v-if="acceptDetailStage">
      <div class="space-y-6 pb-32">
        ...
        <!-- 4. Construction Logs (replaces Before/After evidence) -->
        <div class="p-5 bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div class="flex items-center justify-between mb-4">
            <div class="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2 text-blue-500">
              <CalendarOutlined /> Thông tin nhật ký thi công
            </div>
            <a-button type="primary" ghost size="small" class="rounded-lg text-[10px] font-bold px-3 border-blue-200" @click="showAcceptDetailDrawer = false; activeTabGroup = 'monitor'; activeTab = 'logs'">
              Đến nhật ký <RightOutlined class="text-[9px]" />
            </a-button>
          </div>
          
          <div class="space-y-3">
            <template v-if="logs.filter(l => l.task_id === acceptDetailStage.task_id).length === 0">
              <div class="text-center py-6 bg-gray-50 rounded-xl border border-dashed border-gray-200 flex flex-col items-center justify-center">
                <FileTextOutlined class="text-2xl text-gray-300 mb-2" />
                <div class="text-xs font-medium text-gray-400">Chưa có nhật ký nào cho hạng mục này</div>
              </div>
            </template>
            <template v-else>
              <div v-for="log in logs.filter(l => l.task_id === acceptDetailStage.task_id).sort((a,b) => new Date(b.log_date) - new Date(a.log_date))" :key="log.id" 
                   class="p-3 bg-gray-50 rounded-xl border border-gray-100 hover:border-blue-200 transition-colors cursor-pointer group"
                   @click="showAcceptDetailDrawer = false; openLogDetailDrawer(log)">
                <div class="flex justify-between items-start mb-2">
                  <div class="flex items-center gap-2">
                    <span class="text-xs font-bold text-gray-800">{{ fmtDate(log.log_date) }}</span>
                    <a-tag :color="log.weather === 'sunny' ? 'orange' : log.weather === 'rainy' ? 'blue' : 'default'" class="rounded-full border-0 text-[10px] m-0 font-semibold px-2">
                      {{ { sunny: '☀️ Nắng', rainy: '🌧️ Mưa', cloudy: '⛅ Nhiều mây' }[log.weather] || log.weather }}
                    </a-tag>
                  </div>
                  <div class="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-lg border border-blue-100 shadow-sm">
                    Tiến độ: {{ log.completion_percentage }}%
                  </div>
                </div>
                <div class="text-[11px] text-gray-600 line-clamp-2 leading-relaxed">
                  {{ log.notes || 'Không có ghi chú' }}
                </div>
                <div v-if="log.attachments && log.attachments.length" class="mt-2.5 flex items-center gap-1.5 text-[10px] text-gray-500 font-medium">
                  <PictureOutlined class="text-blue-400" /> {{ log.attachments.length }} tệp đính kèm
                </div>
              </div>
            </template>
          </div>
        ...
        <div class="flex gap-2">
          <template v-if="!isAcceptanceLocked">
            <a-button v-if="acceptDetailStage.workflow_status === 'draft' && can('acceptance.update')"
                      type="primary" ghost size="large" class="rounded-xl h-10 px-6 font-bold border-blue-600 text-blue-600" @click="submitAccept(acceptDetailStage)">
              Gửi duyệt
            </a-button>
...
const allSubcontractorPayments = computed(() => {
  const payments = [];
  subcontractors.value.forEach(sub => {
    if (sub.payments) {
      sub.payments.forEach(p => {
        payments.push({ ...p, subcontractor_name: sub.name, subcontractor: sub });
      });
    }
  });
...
const acStatusLabels = { draft: 'Nháp', pending: 'Chờ duyệt', pending_approval: 'Chờ duyệt', customer_paid: 'Đã thanh toán', approved: 'Đã duyệt', confirmed: 'Đã duyệt', rejected: 'Từ chối', cancelled: 'Đã hủy' }
...
const subInfoCols = [
  { title: 'Tên NTP', key: 'name', ellipsis: true },
  { title: 'Danh mục', dataIndex: 'category', width: 120 },
  { title: 'Ngân hàng', dataIndex: 'bank_name', width: 150 },
  { title: 'Tiến độ', key: 'progress', width: 120 },
  { title: 'Thời gian', key: 'dates', width: 150 },
]

const allSubPayCols = [
  { title: 'Nhà thầu phụ', key: 'subcontractor_name', ellipsis: true },
  { title: 'Nội dung TT', key: 'payment_stage', ellipsis: true },
  { title: 'Số tiền', key: 'amount', align: 'right', width: 130 },
  { title: 'Trạng thái', key: 'status', width: 140 },
  { title: 'Ngày TT', key: 'payment_date', width: 110 },
]
...
const previewLocalFile = (file) => {
  const url = file._previewUrl || URL.createObjectURL(file)
  file._previewUrl = url // Cache URL to prevent memory leaks from recreating multiple URLs for the same file
...
const savePayment = () => {
  ...
  let url = `/projects/${props.project.id}/payments`
  if (editingPayment.value) {
    url += `/${editingPayment.value.id}`
...
// Log Detail Drawer
const openSubPaymentDetail = (payment, subcontractor = null) => {
  subPaymentDetail.value = { ...payment, subcontractor: subcontractor || payment.subcontractor };
  showSubPaymentDetailDrawer.value = true;
}

const getSubPayStepCurrent = (status) => {
  if (status === 'draft') return 0;
  if (status === 'pending_management_approval') return 1;
  if (status === 'rejected') return 1;
  if (status === 'pending_accountant_confirmation') return 2;
...
const saveAC = (status = 'draft') => {
  ...
  const data = { ...acForm.value, status }
  ...
  if (editingAC.value) {
    data._method = 'PUT'
    router.post(`/projects/${props.project.id}/additional-costs/${editingAC.value.id}`, data, savingOptions({
      forceFormData: true,
      onSuccess: () => {
...
const submitAC = (ac) => router.post(`/projects/${props.project.id}/additional-costs/${ac.id}/submit`, {}, loadingOptions(`submit-ac-${ac.id}`))
...
const submitAccept = (acceptance) => {
  router.post(`/projects/${props.project.id}/acceptances/${acceptance.id}/submit`, {}, {
    preserveScroll: true,
    onSuccess: () => {
      message.success('Đã gửi phiếu nghiệm thu')
...
const approveSupervisor = (acceptance) => {
  router.post(`/projects/${props.project.id}/acceptances/${acceptance.id}/supervisor-approve`, {}, {
    preserveScroll: true,
    onSuccess: () => {
      message.success('GS đã xác nhận nghiệm thu')
      showAcceptDetailDrawer.value = false
    },
  })
}
...
const approveCustomer = (acceptance) => {
  router.post(`/projects/${props.project.id}/acceptances/${acceptance.id}/customer-approve`, {}, {
    preserveScroll: true,
    onSuccess: () => {
      message.success('Đã duyệt nghiệm thu thành công')
      showAcceptDetailDrawer.value = false
    },
  })
}
  ...
  Modal.confirm({
    ...
    onOk: () => {
      router.post(`/projects/${props.project.id}/acceptances/${acceptance.id}/revert`, {}, {
        preserveScroll: true,
        onSuccess: () => {
          message.success('Đã hoàn duyệt về nháp')
          showAcceptDetailDrawer.value = false
        },
      })
    },
...
const revertACAction = (ac) => {
  Modal.confirm({
    title: 'Xác nhận hoàn duyệt / hủy gửi duyệt',
    content: 'Chi phí phát sinh sẽ được đưa về trạng thái Nháp để bạn có thể chỉnh sửa lại. Bạn có chắc chắn muốn thực hiện?',
    okText: 'Đồng ý',
    cancelText: 'Hủy',
    onOk: () => {
