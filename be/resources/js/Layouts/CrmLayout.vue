  ...
  <a-layout style="min-height: 100vh">
    ...
    >
      ...
      <div class="flex items-center gap-3 px-5 h-16 border-b border-white/10">
        <div class="w-9 h-9 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0 overflow-hidden">
          <img v-if="$page.props.appLogo && !logoLoadError" :src="`/storage/${$page.props.appLogo}`" alt="Logo" class="w-full h-full object-contain p-1" @error="logoLoadError = true" />
          <svg v-else width="22" height="22" viewBox="0 0 35 36" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div v-if="!collapsed" class="overflow-hidden min-w-0">
          <div class="text-white font-bold text-base leading-tight truncate">{{ $page.props.appName || 'BED CRM' }}</div>
          <div class="text-white/40 text-xs truncate">{{ $page.props.appTagline || 'Construction ERP' }}</div>
        </div>
      </div>
