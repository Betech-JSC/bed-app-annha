...
Route::name('crm.')->middleware(['auth:admin'])->group(function () {
    ...
    Route::prefix('projects')->name('projects.')->group(function () {
        ...
        Route::post('/{project}/equipment-rentals/{rental}/confirm-return', [CrmProjectsController::class, 'confirmReturnRental'])->name('equipment-rentals.confirm-return');
        ...
        // Equipment Purchases — DISABLED: Mua sắm chuyển sang module Kho thiết bị (toàn công ty)
        ...
        // Asset Usages — 3-level approval workflow (Sử dụng thiết bị từ kho)
        ...
        // Generate Labor Costs from Attendance (Tổng hợp lương tháng)
        Route::post('/{project}/attendance/generate-labor-costs', [CrmProjectsController::class, 'generateLaborCosts'])->name('attendance.generate-labor-costs');

        // Notification Redirection Routes
        // Ensures that clicking a notification generated by an API or app action redirects to the correct CRM tab
        ...
        Route::get('/{project}/material-bills/{bill}', fn($project) => redirect("/projects/{$project}?tab=materials"))->name('material-bills.show');
        Route::get('/{project}/acceptance/{stage}/items/{item}', fn($project) => redirect("/projects/{$project}?tab=acceptance"))->name('acceptance.items.show');
        Route::get('/{project}/defects/{defect}', fn($project) => redirect("/projects/{$project}?tab=defects"))->name('defects.show');
        Route::get('/{project}/risks/{risk}', fn($project) => redirect("/projects/{$project}?tab=risks"))->name('risks.show');
    });
    ...
    Route::prefix('hr')->name('hr.')->group(function () {
        ...
        Route::post('/employees/{id}/salary', [CrmHrController::class, 'updateSalaryConfig'])->name('employees.salary.update');
