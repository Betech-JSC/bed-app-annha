...
class CrmProjectsController extends Controller
{
            ...
            'financeData' => [
                ...
                'costs' => $project->costs()
                    ...
                    ->with([
                        ...
                        'materialBill' => function($q) {
                            $q->select('id', 'bill_number', 'bill_date', 'notes', 'supplier_id', 'total_amount')
                              ->with([
                                  'supplier:id,name,phone',
                                  'items.material:id,name,unit,code',
                                  'attachments:id,attachable_id,attachable_type,file_name,original_name,file_size,file_url,mime_type,description'
                              ]);
                        },
     ...
     */
    public function recalculateBudget(string $projectId, string $id)
    {
        $project = Project::findOrFail($projectId);
        $budget = ProjectBudget::where('project_id', $project->id)->findOrFail($id);
        
        $syncService = app(\App\Services\BudgetSyncService::class);
        $syncService->syncBudget($budget);

        return back()->with('success', 'Đã cập nhật lại số liệu ngân sách.');
    }
