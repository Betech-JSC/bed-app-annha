...
export default function CreateBudgetItemDetailScreen() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const updatedItems = [...currentItems, itemPayload];

            // Update budget with new items array
            ...
            if (response.success) {
                Alert.alert("Thành công", "Đã thêm hạng mục", [
                    { text: "OK", onPress: () => router.back() }
                ]);
            }
        } catch (error: any) {
            Alert.alert("Lỗi", error.response?.data?.message || "Không thể thêm hạng mục");
        } finally {
            setSubmitting(false);
        }
    };
