...
import { officeKpiApi, OfficeKpi, UpdateOfficeKpiData, VerifyOfficeKpiData } from "@/api/officeKpiApi";
import { Ionicons } from "@expo/vector-icons";
import { ScreenHeader, PermissionGuard } from "@/components";
import { Permissions } from "@/constants/Permissions";
import { usePermissions } from "@/hooks/usePermissions";

export default function OfficeKpiDetailScreen() {
    const { id } = useLocalSearchParams<{ id: string }>();
    const router = useRouter();
    const [kpi, setKpi] = useState<OfficeKpi | null>(null);
    ...
    const loadKpi = async () => {
        try {
            setLoading(true);
            const response = await officeKpiApi.getKpi(parseInt(id!));
            if (response.success) {
    ...
    const handleUpdateProgress = async () => {
        ...
        try {
            setUpdating(true);
            const data: UpdateOfficeKpiData = {
                current_value: parseFloat(newProgress),
            };
            const response = await officeKpiApi.updateKpi(parseInt(id!), data);
            if (response.success) {
    ...
    const handleVerify = async () => {
        try {
            setUpdating(true);
            const data: VerifyOfficeKpiData = {
                status: verifyStatus,
                note: verifyNote
            };

            const response = await officeKpiApi.verifyKpi(parseInt(id!), data);
            if (response.success) {
        ...
        Alert.alert(
            ...
            [
                { text: "Hủy", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            await officeKpiApi.deleteKpi(parseInt(id!));
            ...
            <ScrollView contentContainerStyle={styles.content}>
                <View style={styles.card}>
                    ...
                    {kpi.year ? (
                        <View style={styles.infoRow}>
                            <View style={styles.infoItem}>
                                <Text style={styles.label}>Năm KPI</Text>
                                <Text style={[styles.value, { fontSize: 24, fontWeight: "700", color: "#3B82F6" }]}>{kpi.year}</Text>
                            </View>
                        </View>
                    ) : (
                        <View style={styles.infoRow}>
                    ...
                    <View style={styles.actionButtons}>
                        ...
                        <PermissionGuard permission={Permissions.KPI_VERIFY}>
                ...
                <PermissionGuard permission={Permissions.KPI_DELETE}>
